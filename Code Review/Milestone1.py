...
sns.boxplot(data=data, y='year', x='salary_int', hue='gender',orient='h')
...
fig1, axes = plt.subplots(5, 2, figsize=(10, 15))
axes = axes.flatten()
regions = data['region'].unique()
for i,region in enumerate(regions):
  ax = axes[i]
  subset = data[data['region'] == region]
  sns.boxplot(data=subset, y='year',x='salary_int', ax=ax, order=[2020, 2021, 2022],orient='h')
  ax.set_title(f'Region = {region}')
  ax.set_xlabel('Salary')
  ax.set_ylabel('Year')
  ax.set_xlim(0,500000)
  ax.set_xticklabels(ax.get_xticklabels(), rotation=90)
plt.suptitle("Salary Trend of Regions",fontsize=16)
plt.tight_layout()
st.pyplot(fig1)
